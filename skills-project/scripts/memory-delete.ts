 ...
 *
 *   tsx memory-delete.ts --scope=project --key=...
 *   tsx memory-delete.ts --scope=user    --key=... [--user=<id>]
 *
 * Env:
...
async function main() {
  ...
  let url: string;
  if (scope === "project") {
    url = `${base}/api/memory/project/${encodeURIComponent(projectKey)}/${encodeURIComponent(key)}`;
  } else {
    const userId = required(
      "FEATBIT_USER_ID",
      arg("user") ?? process.env.FEATBIT_USER_ID
    );
    url = `${base}/api/memory/user/${encodeURIComponent(projectKey)}/${encodeURIComponent(userId)}/${encodeURIComponent(key)}`;
  }
